/**
 * Test JWT helpers (cart-service integration suites).
 ...
 * NOTE: cart-service has NO auth middleware, so these tokens are used to
...
export const validUserToken = (id = 'it-cart-user-1', role = 'user'): string =>
  signToken({ id, role });
